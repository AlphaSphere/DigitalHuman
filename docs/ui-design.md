...
| 文案确认页 | `/tasks/{task_id}/script` | 查看识别 / 解析结果，编辑分段，确认最终文案。 |
| 内容风险提示页 | `/tasks/{task_id}/risk-review` | 展示敏感词、授权、隐私和平台规则风险，要求用户修改或确认。 |
| 配音与数字人配置页 | `/tasks/{task_id}/config` | 选择音色、数字人、比例和字幕样式。 |
...
| 授权确认 | 视频、字幕、声音、肖像和图片素材授权提示 | 未勾选时禁止提交，并保存授权确认记录。 |

关键文案：

- 主标题：`创建数字人口播视频`
- 副标题：`上传参考视频自动识别文案，或直接粘贴你已有的字幕 / 口播稿。`
- 授权提示：`请确认你拥有视频、字幕、声音、肖像和图片素材的合法使用授权，且内容可用于 AI 生成和对外发布。`
- 主按钮：`创建任务`
...
│ 来源预览              │ 文案段落编辑器                     │ 检查面板          │
│                      │                                  │                  │
│ ┌──────────────────┐ │ #01 00:00 - 00:04                │ 低置信度片段      │
│ │ 视频/原文预览      │ │ 原文：大家好，今天介绍...          │ 3 个需检查        │
│ │                  │ │ 编辑：[大家好，今天介绍...]        │                  │
│ └──────────────────┘ │                                  │ 文案统计          │
│                      │ #02 00:04 - 00:08                │ 12 段 / 428 字    │
│ 来源：Whisper识别     │ 原文：...                          │                  │
│ 时长：62.5s          │ 编辑：[                  ]         │ 操作记录          │
│                      │                                  │ 已自动保存         │
├──────────────────────┴──────────────────────────────────┴──────────────────┤
...
| 模块 | 内容 | 交互 |
| --- | --- | --- |
| 音色选择 | 音色卡片、试听按钮、provider 标记 | 调用 `GET /api/voice-profiles`。 |
| 数字人选择 | 头像 / 预览图、名称、比例能力 | 调用 `GET /api/avatar-profiles`。 |
...
- 上传失败、文案为空、授权未确认都有明确中文提示。
- 文案确认页可以展示、编辑、保存和确认段落。
- 内容风险提示页可以展示风险类型、命中位置、处理建议，并区分可确认和不可放行风险。
- 配置页可以选择音色、数字人、输出比例和字幕样式。
